...
public class PPS200B extends BasePage {
   ...
   */
  public void CreatePO(final String PONo) {
    this.txtPONo.click();
    BasePage.clearRobustly(this.txtPONo);
    this.txtPONo.sendKeys(PONo);
  }
   ...
   * @param: PO number
   */
  public void openPO(final String PONo) {
    boolean _SearchPONo = this.SearchPONo(PONo);
    if (_SearchPONo) {
      DefaultWebDriver.driver.findElement(By.xpath((("//a[text()=\'" + PONo) + "\']"))).click();
    }
  }
  
  public String verifyLowerStatus(final String PONo) {
    this.CreatePO(PONo);
    this.txtPONo.sendKeys(Keys.ENTER);
    BasePage.waitForLoadingComplete();
    InforGrid grid = new InforGrid(this.gridElement);

...
class MWS460B extends BasePage{
	...
	@FindBy(css="div[id*='MWA460BS'][class*='inforDataGrid']") 
 	...
 	@FindBy(id = "W2OBKV")
 	WebElement txtLocationSearch
 	
 	@FindBy(xpath = "(//a/span[text()='Balance ID - reclassify'])[3]")
	WebElement linkBalanceIDReclassify
	
	def void selectSortingOrder(String sorting){
		selectFromDropdown(btnSortingSelect,listOptions,sorting)
	}
	...
	def void selectView(String view){
		selectFromDropdown(btnViewSelect,listOptions,view)
	}
	
	def void searchLocation(String location){
		txtLocationSearch.EnterText(location)
		txtLocationSearch.sendKeys(Keys.ENTER)
	}
	
	def void balanceIdReclassify(){
		var InforGrid grid= new InforGrid(gridElement);
		...
		linkRelated.click()
		linkBalanceIDReclassify.waitToBeClickable()
		linkBalanceIDReclassify.click()
		waitForLoadingComplete()
	}
}
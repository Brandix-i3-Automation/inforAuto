...
public class EndToEndMadeToOrderTest extends BaseTest {
	...
	def void Initialize() {
		...
		mms060e = new MMS060E(driver)
		mms130a = new MMS130A(driver)
		mms130e = new MMS130E(driver)
	}
	...
	def void EndToEndMadeToOrderTest() {
		...
		var String location = "YPF001"
		var String warehouse = "001"
		/* Test*/
...
// 		verify for status 90	
	...
	def void pendingPutAwayAndReclassify(String itemNumber,String location,String warehouse){
		homePage.GoToMWS060()
		mws060b.enterItemNumber(itemNumber)
		mws060b.changeFirstRecord()
		var pendingPutAway = mms060e.getPendingPutAwayValue()
		if(!pendingPutAway.equals("0")){
			mms060e.closeAllTabs()
			homePage.GoToMWS460()
			mws460b.selectSortingOrder("10-Loc,Item.Lot")
			mws460b.selectView("F10-Warehouse, Location, Item, Lot")
			mws460b.setWarehouse(warehouse)
			mws460b.setItemNumber(itemNumber)
			mws460b.searchLocation(location)
			mws460b.processGridRow(0)
			Assert.assertTrue(mws460b.getFooterMessage().contains("is processed"))
			mws460b.closeAllTabs()
			homePage.GoToMWS060()
			mws060b.enterItemNumber(itemNumber)
			mws060b.changeFirstRecord()
			pendingPutAway = mms060e.getPendingPutAwayValue()
			Assert.assertEquals(pendingPutAway,"0")
		}
		mms060e.clickNext()
		
		//Reclassifying
		mws460b.balanceIdReclassify()
		mms130a.setItemNumber(itemNumber)
		mms130a.setCalcMethod("1-Change status location level")
		mms130a.clickNext()
		mms130e.checkAllocatable()
		mms130e.clickNext()
		mms130a.closeAllTabs()
	}

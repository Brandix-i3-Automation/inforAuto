...
class DistributionOrderFlowTest extends FnBCommons {
	...
	def void DistributionOrderFlowTest() {
		...
		pps310a.SetRecievingNo(ReceivingNo);//This step is not mandatory as per Eshan. however added due to random issue
		pps310a.GoToPPS310E();
		...
		// ----Added this code to retry release for picking function if RIP !=1 ----------------------------------------
		var String RIP = mws410b.TakeRipValue().get(0);
		if (RIP != 1) {
			Thread.sleep(2000);
			mws410b.goToRelatedOption("Release for Picking");
			Thread.sleep(6000);
		...
		RIP = mws410b.TakeRipValue().get(0);
		Assert.assertEquals(RIP, "1", "RIP did not change to 1 after 2 retry's. Order number is: " + newPoNumber);
		// -----End Retry code---------------------------------------
